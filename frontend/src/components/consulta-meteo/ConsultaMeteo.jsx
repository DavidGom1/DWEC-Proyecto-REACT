...
export default function ConsultaMeteo({ isDark, idProvincia, idMunicipio }) {
            ...
            try {
                if (idMunicipio != null) {
                    ...
                    if (!json.success || !json.data) {
                        setDatos({
                            nombreProvincia: 'Error',
                            descripcion: json.error || 'No se pudieron obtener los datos.',
                            temps: []
                        });
                        return;
                    }
    ...
    return (
        <div className={`rounded-[20px] w-full lg:w-4xl md:w-2xl overflow-hidden shadow-lg border ${isDark ? 'bg-[#2d2d2d] border-[#1d1d1d] text-gray-200' : 'bg-white border-gray-200 text-gray-800'
            }`}>
